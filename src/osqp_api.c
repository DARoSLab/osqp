                   ...
                   const OSQPSettings *settings)
{
  ...
  // Validate data
  if (validate_data(P, q, A, l, u, m, n))
...
OSQPInt osqp_solve(OSQPSolver *solver)
{
  ...
  if (!solver->settings->warm_starting)
    osqp_cold_start(solver);
  ...
  for (iter = 1; iter <= max_iter; iter++)
  {
    ...
    if (can_check_termination || can_print || iter == 1)
  ...
  /* Print final footer */
  if (solver->settings->verbose)
    print_footer(solver->info, solver->settings->polishing);
#endif /* ifdef OSQP_ENABLE_PRINTING */
...
OSQPInt osqp_cleanup(OSQPSolver *solver)
{
  ...
  { // If workspace has been allocated
    ...
    // Free timer
    if (work->timer)
      OSQPTimer_free(work->timer);
#endif /* ifdef OSQP_ENABLE_PROFILING */

#ifdef OSQP_ENABLE_DERIVATIVES
    if (work->derivative_data)
    {
      if (work->derivative_data->y_l)
        OSQPVectorf_free(work->derivative_data->y_l);
      if (work->derivative_data->y_u)
        OSQPVectorf_free(work->derivative_data->y_u);
      if (work->derivative_data->ryl)
        OSQPVectorf_free(work->derivative_data->ryl);
      if (work->derivative_data->ryu)
        OSQPVectorf_free(work->derivative_data->ryu);
      if (work->derivative_data->rhs)
        OSQPVectorf_free(work->derivative_data->rhs);
                             ...
                             const OSQPFloat *u_new)
{
  ...
  if (l_new || u_new)
  {
    ...
    if (l_new)
      swap_vectors(&work->z_prev, &work->data->l);
    if (u_new)
      swap_vectors(&work->delta_y, &work->data->u);
                        ...
                        const OSQPFloat *y)
{
  ...
  if (!solver->settings->warm_starting)
    solver->settings->warm_starting = 1;
                     ...
                     OSQPCodegenDefines *defines)
{
  ...
  else if (!defines || (defines->embedded_mode != 1 && defines->embedded_mode != 2) || (defines->float_type != 0 && defines->float_type != 1) || (defines->printing_enable != 0 && defines->printing_enable != 1) || (defines->profiling_enable != 0 && defines->profiling_enable != 1) || (defines->interrupt_enable != 0 && defines->interrupt_enable != 1) || (defines->derivatives_enable != 0 && defines->derivatives_enable != 1))
  {
  ...
  if (!exitflag)
    exitflag = codegen_src(output_dir, file_prefix, solver, defines->embedded_mode);
  if (!exitflag)
    exitflag = codegen_example(output_dir, file_prefix);
  if (!exitflag)
    exitflag = codegen_defines(output_dir, defines);
#else
